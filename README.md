...
在 AIDC 环境完成基础建设后，GPU 服务器常常面临一个关键问题：  
**业务上线前如何快速确认网络链路是否真正可用、带宽是否达标。**

本工具的核心目标就是把这一步标准化、可视化：
- 对 GPU/CPU 服务器做**以太网连通性与带宽检查**（Ping + iperf）
- 对 InfiniBand 网络做**拓扑与带宽/延迟检查**
- 将测试过程与结果沉淀为可追溯的日志，便于验收、排障和复测对比

它更像是一套轻量的“上架验收基线工具”，帮助你在 AIDC 网络打通后，快速判断 GPU 集群是否具备稳定交付条件。
...
| **以太网测试** | GPU/CPU 带宽测试 | 基于 iperf 测试 GPU/CPU 主机以太网带宽，支持单对与批量模式（按网口规格判定阈值） |
...
基于远程主机上安装的 `iperf`/`iperf3` 工具运行测试，聚合带宽阈值判定建议为：
- 物理口为 **两条 10G 接口**：聚合带宽 >= **18 Gbits/sec**
- 物理口为 **两条 25G 接口**：聚合带宽 >= **46 Gbits/sec**
